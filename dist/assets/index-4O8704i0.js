...
function Player({ playerRef, portals = [], onPortalEnter, onProximityChange, onPortalsProximityChange, onNearPortalChange, navigateToPortalId = null, onReachedPortal, onOrbStateChange, onHomeSplash, onCharacterReady, sceneColor }) {
  const { gl } = useThree();
    ...
    const update = () => {
      ...
      try {
        ...
        cards.forEach((el) => {
          ...
          const shadowA = 0.1 + 0.25 * (1 - ease(t));
...
function ParallaxBirds({ scrollerRef }) {
  ...
  useFrame((_, delta) => {
    ...
    const tL = clamp01((tRaw - 0.05) / 0.9);
    const tR = clamp01((tRaw - 0.3) / 0.65);
    ...
    if (!initRef.current) {
      ...
      whiteYRef.current = Math.min(viewportH * 0.94, yWscreenTarget + viewportH * 0.1);
      whiteVyRef.current = 0;
      leftXRef.current = layout.left.x - size.width / 2;
      leftVxRef.current = 0;
      rightXRef.current = layout.right.x - size.width / 2;
      rightVxRef.current = 0;
      whiteXRef.current = layout.white.x - size.width / 2;
      whiteVxRef.current = 0;
    ...
    }
    {
      const time = (typeof performance !== "undefined" ? performance.now() : Date.now()) / 1e3;
      {
        ...
        leftYRef.current += Math.sin(time * 0.25 + 0.3) * 0.02 * viewportH * dt2;
      }
      {
        ...
        rightYRef.current += Math.cos(time * 0.22 + 1.1) * 0.018 * viewportH * dt2;
      }
      {
        ...
        if (xNext < minX) {
          const over = minX - xNext;
          xNext = minX + over;
          vxNext = Math.max(Math.abs(vxNext) * bounceX + 0.26 * Math.abs(vx), minKickX);
        } else if (xNext > maxX) {
          const over = xNext - maxX;
          xNext = maxX - over;
          vxNext = -Math.max(Math.abs(vxNext) * bounceX + 0.26 * Math.abs(vx), minKickX);
        ...
        whiteYRef.current += Math.sin(time * 0.28 + 2) * 0.016 * viewportH * dt2;
      }
    }
    if (leftRef.current) {
      const x = leftXRef.current;
    ...
    if (whiteRef.current) {
      ...
      let y = viewportH / 2 - whiteYRef.current;
      const ENTRY_MS = 1e3;
      const now = typeof performance !== "undefined" ? performance.now() : Date.now();
      if (entryStartRef.current == null) entryStartRef.current = now;
      const p = Math.max(0, Math.min(1, (now - entryStartRef.current) / ENTRY_MS));
      if (p < 1) {
        const offY = viewportH * 0.4;
      ...
      whiteRef.current.rotation.y += delta * (0.052 + 0.02 * Math.sin(tW * PI2) + entryBoostW);
      whiteRef.current.rotation.x += delta * (0.017 + 0.011 * Math.cos(tW * PI2 * 0.9) + entryBoostW * 0.5);
      whiteRef.current.rotation.z += delta * (0.02 + 9e-5 * vel) + -whiteVyRef.current * 33e-6 + entryBoostW * 0.33;
    }
    const resolvePair = (aRef, bRef, aXR, aYR, aVX, aVY, bXR, bYR, bVX, bVY, rA, rB, lastRef) => {
      ...
      const vAn = aVX.current * nx + -aVY.current * ny;
...
function App() {
  ...
  reactExports.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        addTask(() => fetch(`${"/development/"}songs/manifest.json`, { cache: "no-cache" }).then((r) => r.json()).then((list) => {
          try {
            const arr = Array.isArray(list) ? list : [];
            arr.forEach((file) => addTask(() => fetch(`${"/development/"}songs/${file}`, { cache: "force-cache" }).then((r) => r.blob())));
          } catch {
          }
        }));
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-full h-full relative overflow-hidden", children: [
    ...
    bootLoading && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "fixed inset-0 z-[20000] bg-[#0a0f22] text-white", role: "dialog", "aria-modal": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 w-full h-full", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center justify-center p-8 col-span-1 md:col-span-1 md:justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-full max-w-xl text-center md:text-left", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "font-marquee uppercase text-[2.625rem] sm:text-[3.15rem] md:text-[4.2rem] leading-[0.9] tracking-wide mb-4", style: { WebkitTextStroke: "1px rgba(255,255,255,0.08)" }, children: "La Última Chamba de Skulley Rad" }),
        ...
        !(bootAllDone && characterReady) && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "mt-2", children: [
        ...
        bootAllDone && characterReady && /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
            onClick: () => {
