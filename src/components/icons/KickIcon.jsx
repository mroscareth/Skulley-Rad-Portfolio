import React from 'react'

// Tenis en movimiento (patada). Heroicons no trae nada de pie/bota/huella —
// mismo caso que GamepadIcon, que tampoco existía ahí. Este sale de Phosphor
// Icons (`sneaker-move`, peso fill), MIT — https://phosphoricons.com
//
// Se probaron antes el `shoe-prints` de Font Awesome (dos zapatos: lee como
// caminar, no como patada) y una huella dibujada a mano. Este es un solo pie
// inclinado con líneas de velocidad: la lectura correcta.
export default function KickIcon({ className = '' }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 256 256"
      fill="currentColor"
      className={className}
      aria-hidden="true"
      focusable="false"
    >
      <path d="M70.8,184H32a8,8,0,0,1,0-16H70.8a8,8,0,1,1,0,16Zm32,16H48a8,8,0,0,0,0,16h54.8a8,8,0,1,0,0-16Zm128.36-33.37-28.63-14.31A47.74,47.74,0,0,1,176,109.39V80a8,8,0,0,0-7.93-8A48.05,48.05,0,0,1,120,24.07a8,8,0,0,0-12.83-6.44L45.11,64.68a4,4,0,0,0-.41,6l51.44,51.44a8.19,8.19,0,0,1,.6,11.09,8,8,0,0,1-11.71.43l-53-53a4,4,0,0,0-6.44,1.09,16,16,0,0,0,3.12,18.22L142.4,213.66a8,8,0,0,0,5.66,2.34H224a16,16,0,0,0,16-16V180.94A15.92,15.92,0,0,0,231.16,166.63Z" />
    </svg>
  )
}
